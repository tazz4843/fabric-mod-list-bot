...
class PermissionsCommands(Cog):
    ...
    async def view_member(self, ctx: MyContext, member: discord.Member = None):
        """
        Display all _permissions set for this channel
        """
        _ = await ctx.get_translate_function()
        ...
        _permissions = db_member.permissions

        if len(_permissions):
            message = [_("**{member.name}#{member.discriminator} _permissions**", member=member), "```diff"]

            for permission, value in _permissions.items():
                sign = "+" if value else "-"
                message.append(f"{sign} {permission}")
            message.append("```")
            await ctx.send("\n".join(message))
        else:
            await ctx.send(_("There are no specific _permissions for this member."))
    ...
    async def set_member(self, ctx: MyContext, member: discord.Member, permission: str, value: bool):
        ...
        await db_user.save(update_fields=['permissions'])
        await ctx.send(_(
            "👌 Permission {permission} for member {member.name}#{member.discriminator} has "
            "been set to {value} globally.",
            permission=escape_everything(permission),
            member=member,
            value=value))
