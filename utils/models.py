...
async def get_from_db(discord_object, as_user=False):
    if isinstance(discord_object, discord.Guild):
        ...
        db_obj = await DiscordMember.filter(user__discord_id=discord_object.id).first().\
...
async def get_ctx_permissions(ctx: 'MyContext') -> dict:
    ...
    permissions = {**default_permissions, **subguild_permissions, **subchannel_permissions, **member_permissions,
                   **fixed_permissions, **user_permissions}
    return permissions
